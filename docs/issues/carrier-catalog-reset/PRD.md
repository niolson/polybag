...
- **Direct integration is a column, not a name lookup.** `carriers.adapter` is a nullable,
  unique key (`usps`, `ups`, `fedex`, `amazon_shipping`), and `CarrierRegistry` is keyed
  by it. Null means no direct integration:
  ...
  sell them. The column is visible in queries and the UI, and renaming a carrier no
  longer breaks rating.
- **Seeders find carriers by the key.** `CarrierSeeder` runs on every start through
  `app:sync-reference-data`, and finds carriers by name today, so a renamed UPS comes back
  as a second row. Carriers with no adapter are found by name, which is all they have. A
...
7. **Which carriers without a direct integration, and which of their services, get
   ...
   carry directly are authored under those carriers. Direct integration is recorded in a
   `carriers.adapter` column (see [Catalog](#catalog)). *Revised in the second review:*
   Amazon Shipping has one, `amazon_shipping`, reached through the scoped connection.
   OnTrac and DHL Express do not.
