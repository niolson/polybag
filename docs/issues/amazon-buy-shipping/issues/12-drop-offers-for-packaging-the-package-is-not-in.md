...
Status: done — shipped 2026-09-15 as `fitsThePackaging()` in the Amazon adapter; rules 1–2 moved to the classifier in `packaging-form-and-carrier-identity/04` on 2026-09-16
...
### 2026-09-16 — rules 1 and 2 moved
...
### 2026-09-25 — rule 3 replaced, and the evidence corrected

> *This was generated by AI during implementation.*

The reason given above, that "the delivery promise has excluded them so far", was wrong.
Six of the eight captures in `.scratch/amazon-shipping-v2/` that list these services refuse
`USPS_PTP_MM` and `USPS_PTP_BPM` for "The shipping service is not available for the
products in the order." That is a product reason: Amazon checks the order's products for
them. The other two, the `probe-09` quotes for `112-2122617-2225806` and
`114-6201360-7304251`, refuse both with only the generic "The selected shipping service
cannot be purchased; please try a different selection…". None of the eight has returned
...
- Media Mail is dropped only for a Package that does not qualify. The requirement comes
  from the mapped `CarrierService`, or from the identifier until `carrier-catalog-reset/11`
  maps it.
- `USPS_PTP_BPM` is shown but never bought unattended.
- Both UPS SurePost identifiers stay dropped. `UPS_PTP_SUREPOST_MEDIA` stays dropped only
  until `11` maps it.
