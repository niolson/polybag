...
Status: wontfix — 2026-09-23: speed comes from the due-by date (`17`), not from a list of catalog services
...
Built, then withdrawn before merging. The work is kept in a local stash, not in the repo.
Three reasons:

- **It guarded the wrong direction.** The harm is buying too *slow* a service. A faster
  service that happens to be cheaper is fine. A delivery deadline already guards against
  too slow: `deliver_by` on an Amazon order, the method's `commitment_days` otherwise
  (`Shipment::getDeliverByDate()`). `17` puts *exclude rates that deliver after the
  due-by date* on the shipping method, for every order on it.
- **It needed a mapping for every offer.** An offer counted as in the class only when
  mapped to a `CarrierService` the method lists. OnTrac and DHL have no catalog rows, so
  they could never pass a constrained method.
- **Configuring it was unintuitive.** The method had to list the `AMAZON_BUY_SHIPPING`
  row *and* the direct services whose Amazon equivalents should pass. Listing those also
  asked any configured direct account for them.

One finding from the attempt for whoever revisits this: the Amazon adapter creates its
`ShippingOffer` rows inside `getRates()`. A post-quote filter in `ShippingRateService`
alone hides a rate but leaves its Offer behind, so a filter like this has to run in the
adapter too, as `fitsThePackaging()` does.
