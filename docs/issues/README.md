...
work, which is measurement and can wait. `20` closed the same day; `21` (with `18`) is next.
...
2. [`21`](shopify-shipping-carrier/issues/21-a-void-makes-the-next-import-create-a-duplicate-shipment.md),
   ...
   Void a label, re-import, and the packing queue holds two open shipments for one order, one
   of which can never buy a label. One root cause seen from two ends; the import-side fix
   covers both, and fixing `18` alone leaves the duplicate. Do them as one change, `21` first.
3. [`19`](shopify-shipping-carrier/issues/19-international-purchase-fails-when-the-box-weighs-less-than-its-contents.md)
   — narrower than the other two (international only, and Shopify postage is a per-client
