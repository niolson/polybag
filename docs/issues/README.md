...
4. ~~[`01`](shopify-shipping-carrier/issues/01-verify-first-live-label-purchase.md)~~ —
   **done 2026-09-09.** Questions 2, 4, 6 and 9 all closed that day, the last of them by
   ...
   Question 4 answered **no duplicate notification**, but only because the second call site
   cannot succeed: that is
   [`20`](shopify-shipping-carrier/issues/20-every-shopify-bought-package-lands-a-permanent-export-failure.md),
   where `ShopifySource::exportPackage()` swallows only `already fulfilled` and Shopify
   actually says *"unfulfillable status= closed"*, so every Shopify-bought package records a
   `permanently_failed` export. That also corrects question 3's recorded inference that export
   "degrades safely"; it does not. Fixing `20` so `fulfillmentCreate` succeeds would reopen
   question 4. Question 4 answered **no duplicate notification**, but only
   because the second call site cannot succeed: that is
   [`20`](shopify-shipping-carrier/issues/20-every-shopify-bought-package-lands-a-permanent-export-failure.md),
   where `ShopifySource::exportPackage()` swallows only `already fulfilled` and Shopify
   actually says *"unfulfillable status= closed"*, so every Shopify-bought package records a
   `permanently_failed` export. That also corrects question 3's recorded inference that export
   "degrades safely"; it does not. Fixing `20` so `fulfillmentCreate` succeeds would reopen
   question 4. Question 2: there is no ZPL
