...
3. **[`07`](shopify-shipping-carrier/issues/07-customs-form-printing.md)'s pre-purchase
   ...
   Amazon row. Since 2026-09-16 that row is filled provisionally from Amazon's own
   statement (separate `CUSTOM_FORM`, declared per offering), so the Amazon half of the
   gate is buildable now as
   [`amazon-buy-shipping/13`](amazon-buy-shipping/issues/13-handle-international-offers-from-the-rate-response.md),
   `ready-for-agent`; the observation itself still waits on
   ...
   suspected — and which `13` avoids by gating on the offering.
