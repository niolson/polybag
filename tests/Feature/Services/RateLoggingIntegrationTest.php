...
it('cascade deletes rate quotes when package is deleted', function () {
...
it('creates rate quotes with factory', function () {
...
it('creates selected rate quote with factory state', function () {
    $quote = RateQuote::factory()->selected()->create();

    expect($quote->selected)->toBeTrue();
});

it('relates rate quotes to package', function () {
    $package = Package::factory()->create();
